...
public class SoccerBall extends AbstractEntity {

	public SoccerBall(Vector3d pos) {
		add(new Position(pos));
		add(new Rotation());
		add(new RendereableComponent());
		add(new Scale());
		getComponent(RendereableComponent.class).model = EntityResources.getSoccerBall();
	}
}
