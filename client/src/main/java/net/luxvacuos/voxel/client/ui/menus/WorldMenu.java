...
public class WorldMenu extends RootComponent {
	...
	public void initApp(Window window) {
		...
		ScrollPane pane = new ScrollPane(0, 0, 420, appH, 385f, 60f);
		pane.setColls(1);
		...
		try {
			Files.walk(worldPath.toPath(), 1).forEach(filePath -> {
				if (Files.isDirectory(filePath) && !filePath.toFile().equals(worldPath)) {
					pane.addElement(new WorldElement(385, 60, filePath.getFileName().toString(), this));
				}
			});
		} catch (IOException e) {
			e.printStackTrace();
		...
		Button loadButton = new Button(-210, 60, 200, 40, "Load World");
